...
export default function PageIdAnime() {
    const { idA } = useParams()
    ...
    useEffect(() => {
        const fetchAnimeGenre = async () => {
            ...
            setError(null)
            try {
                const url = `https://api.jikan.moe/v4/anime?genres=${idA}&page=${page}&limit=25`
                ...
                const genre = genresRs.data.data.find(g => g.mal_id === parseInt(idA))
